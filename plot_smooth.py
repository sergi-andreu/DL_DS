...
for iii in range(1,nlayers):
    ...
    for c in smoother:
        ...
        if dim==2:
            ax.scatter(c*np.transpose(activations)[0]+(1-c)*activations0[0], c*np.transpose(activations)[1]+(1-c)*activations0[1], c=colors, s=size)

        if dim==3:
            ax.scatter(c*np.transpose(activations)[0]+(1-c)*activations0[0], c*np.transpose(activations)[1]+(1-c)*activations0[1], c*np.transpose(activations)[2]+(1-c)*activations0[2], c=colors, s=size)
        ...
        if CARTESIANGRID:
            ...
            if dim==2:
                for i in range(10):
                    ...
                    ax.plot(c*act[0]+(1-c)*act0[0], c*act[1]+(1-c)*act0[1], c="k", linewidth=linewidth)
            ...
            if dim==3:
                for i in range(10):
                    ...
                    ax.plot(c*act[0]+(1-c)*act0[0], c*act[1]+(1-c)*act0[1], c*act[2]+(1-c)*act0[2], c="k", linewidth=linewidth)
        ...
        fig.savefig("2D_ResNet_Ring/im"+str((iii-1)*(ccc-1)+int(ccc*c)).zfill(2)+".png",bbox_inches='tight')
    ...
    if CARTESIANGRID:
        act0 = act[:]
    activations0 = np.transpose(activations)[:]